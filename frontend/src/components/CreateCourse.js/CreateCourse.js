...
function CreateCourse(props) {
  ...
  const handleCourseCreation = async () => {
    ...
    try {
      await Axios.post("http://localhost:4000/api/course", {
      ...
      let course = await Axios.get('http://localhost:4000/api/users?type=courses', {
        headers: {
          "Authorization": `Bearer ${localStorage.getItem('token')}`,
        }
      });
  ...
  return (
    <div>
      <Dialog open={props.newCourse.open} onClose={handleClose} aria-labelledby="form-dialog-title">
        <DialogTitle id="form-dialog-title">Course</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Provide the following details and complete the process of creation of class.
        ...
        <DialogActions>
          ...
          <Button
            ...
            color="primary"
            disabled={!(props.newCourse.courseID && props.newCourse.description.length > 100 && props.newCourse.name)}
          >
            Create Class
          </Button>
        </DialogActions>
