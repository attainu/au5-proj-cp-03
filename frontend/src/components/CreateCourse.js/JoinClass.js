...
function JoinClass(props) {
  ...
  const handleJoinClass = async (event) => {
    ...
    try {
      await Axios.post("http://localhost:4000/api/users/course", {
  ...
  console.log(props.joinClass);
  return (
    <div>
      <Dialog open={props.joinClass.open} onClose={handleClose} aria-labelledby="join-class">
        ...
        <DialogContent>
          ...
          <TextField
            error={props.joinClass.enrollString && props.joinClass.enrollString.length !== 20}
